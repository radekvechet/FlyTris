...
const read=f=>fs.readFileSync(path.join(data,f),'utf8'),d=JSON.parse(read('results.json'));
const mean=games=>games.reduce((n,g)=>n+g.lines,0)/games.length;
const values={TABLE:read('report-table.html'),RAW:mean(d.raw_control).toFixed(2),RANDOM:mean(d.random_control).toFixed(2),DATA:read('replay.json'),SHAPES:read('shapes.json'),VERSUS_MODEL:read('versus-model.json'),NEURONS:d.model.neurons,EDGES:d.model.edges,STATUS:d.replicates.every(r=>r.summary.learning_evidence)?'Learning observed in every replicate':'Learning evidence needs review'};
for(const [key,file] of Object.entries({STYLE:'style.css',TIMELINE:'replay.js',SCENE:'scene.js',BOOT:'boot.js',THREE_LIB:'third_party/three.cjs',THREE_LICENSE:'third_party/THREE-LICENSE.txt',VERSUS_STYLE:'versus.css',VERSUS_HTML:'versus.html',VERSUS_CORE:'versus-core.js',VERSUS_UI:'versus.js',SCORES_HTML:'scores.html',SCORES_STYLE:'scores.css',SCORES_UI:'scores.js'}))values[key]=fs.readFileSync(path.join(web,file),'utf8');

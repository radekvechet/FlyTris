...
function makeService(db,modelHash,now=Date.now){
  ...
  return {
    ...
    async leaderboard(window='24h',difficulty='all'){
      ...
      let where='rules_version=3 AND completed_at IS NOT NULL AND completed_at >= $1';if(difficulty!=='all'){where+=' AND difficulty=$2';params.push(difficulty);}
      ...
      for(const level of Object.keys(PRESETS)){
        const rows=await db.query(`SELECT id,player_name,human_lines,human_pieces,fly_lines,fly_pieces,winner,completed_at FROM flytris_matches WHERE rules_version=3 AND difficulty=$1 AND completed_at >= $2 ORDER BY human_lines DESC,human_pieces DESC,completed_at ASC,id ASC LIMIT 10`,[level,cutoff]);

...
const PRESETS={easy:{gravityMs:800,flyMs:800,durationMs:180000,rulesVersion:2},medium:{gravityMs:500,flyMs:500,durationMs:180000,rulesVersion:2},hard:{gravityMs:250,flyMs:250,durationMs:180000,rulesVersion:2}};
...
function makeService(db,modelHash,now=Date.now){
  ...
  return {
    async start(input){
      ...
      await db.query('INSERT INTO flytris_matches (id,token_hash,difficulty,model_hash,seed,started_at,rules_version) VALUES ($1,$2,$3,$4,$5,$6,2)',[id,hash(token),input.difficulty,modelHash,seed,started]);
    ...
    async finish(input){
      ...
      if(Number(s.rules_version)!==2)fail('This match uses retired rules. Start a new match.',409);
      ...
      if(!r.settings||r.settings.seed!==Number(s.seed)||r.settings.gravityMs!==p.gravityMs||r.settings.flyMs!==p.flyMs||r.settings.durationMs!==p.durationMs||r.settings.rulesVersion!==2||r.model?.model_sha256!==s.model_hash)fail('Match settings changed.');
    ...
    async leaderboard(window='24h',difficulty='all'){
      ...
      let where='rules_version=2 AND completed_at IS NOT NULL AND completed_at >= $1';if(difficulty!=='all'){where+=' AND difficulty=$2';params.push(difficulty);}
      ...
      for(const level of Object.keys(PRESETS)){
        const rows=await db.query(`SELECT id,player_name,human_lines,human_pieces,fly_lines,fly_pieces,winner,completed_at FROM flytris_matches WHERE rules_version=2 AND difficulty=$1 AND completed_at >= $2 ORDER BY human_lines DESC,human_pieces DESC,completed_at ASC,id ASC LIMIT 10`,[level,cutoff]);

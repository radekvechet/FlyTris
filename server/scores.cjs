...
const PRESETS={easy:{gravityMs:800,flyMs:5000,durationMs:180000},medium:{gravityMs:500,flyMs:3000,durationMs:180000},hard:{gravityMs:250,flyMs:1500,durationMs:180000}};
...
function validateStats(s){
  if(!s||!integer(s.lines,0,800)||!integer(s.pieces,0,2000)||typeof s.alive!=='boolean')fail('Invalid score.');
  if(!Array.isArray(s.clears)||s.clears.length!==4||!s.clears.every(v=>integer(v,0,2000))||s.clears.reduce((n,v,i)=>n+v*(i+1),0)!==s.lines)fail('Invalid row-clear totals.');
...
function makeService(db,modelHash,now=Date.now){
  async function session(input){
    if(typeof input.id!=='string'||typeof input.token!=='string'||input.token.length!==64)fail('Invalid match token.',403);
  ...
  return {
    async start(input){
      if(!Object.hasOwn(PRESETS,input.difficulty))fail('Choose Easy, Medium or Hard.');
      ...
      await db.query('INSERT INTO flytris_matches (id,token_hash,difficulty,model_hash,seed,started_at) VALUES ($1,$2,$3,$4,$5,$6)',[id,hash(token),input.difficulty,modelHash,seed,started]);
    ...
    async finish(input){
      ...
      if(!r.settings||r.settings.seed!==Number(s.seed)||r.settings.gravityMs!==p.gravityMs||r.settings.flyMs!==p.flyMs||r.settings.durationMs!==p.durationMs||r.model?.graph_sha256!==s.model_hash)fail('Match settings changed.');
      ...
      if(r.fly.pieces>Math.ceil(ms/p.flyMs)||r.human.pieces>Math.ceil(ms/40))fail('Score exceeds the match pace.');
    ...
    async leaderboard(window='24h',difficulty='all'){
      ...
      let where='rules_version=1 AND completed_at IS NOT NULL AND completed_at >= $1';if(difficulty!=='all'){where+=' AND difficulty=$2';params.push(difficulty);}
      ...
      for(const level of Object.keys(PRESETS)){
        const rows=await db.query(`SELECT id,player_name,human_lines,human_pieces,fly_lines,fly_pieces,winner,completed_at FROM flytris_matches WHERE rules_version=1 AND difficulty=$1 AND completed_at >= $2 ORDER BY human_lines DESC,human_pieces DESC,completed_at ASC,id ASC LIMIT 10`,[level,cutoff]);

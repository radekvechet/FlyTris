...
function initial(run){return {human:L.snapshot(new V.FallingPlayer(new V.Sequence(run.seed),shapes,run.gravityMs),run.seed),fly:L.snapshot(new V.FallingPlayer(new V.Sequence(run.seed),shapes,run.flyMs??run.gravityMs),run.seed),elapsedMs:0,gravityDue:false,controls:0,hardDrops:0,reason:null};}
...
function advance(saved,commands,run){
  ...
  for(const command of commands){
    ...
    if(command==='B'){
      if(state.gravityDue||state.elapsedMs>=run.durationMs)fail('Invalid game clock.');
      ...
      state.reason=!human.alive?'human-topout':!fly.alive?'fly-topout':state.elapsedMs===run.durationMs?'time':null;
    }else{

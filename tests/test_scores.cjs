...
function result(s){return {settings:{...s.settings,seed:s.seed},model:{model_sha256:'model-test'},elapsedSeconds:120,reason:'time',human:stats(),fly:stats(1,4)};}
...
test('one timestamped match atomically records both scores; retries do not duplicate',async()=>{
  const f=await fixture();try{const s=await f.service.start({difficulty:'medium',rulesVersion:3,modelHash:'model-test'});f.add(120000);
...
test('time windows, difficulty lists and chart filter use completed timestamps',async()=>{
  ...
  try{for(const [age,difficulty] of [[40,'easy'],[29,'medium'],[6,'hard'],[0,'easy']]){f.set(end-age*86400000-120000);const s=await f.service.start({difficulty,rulesVersion:3,modelHash:'model-test'});f.add(120000);await f.service.finish({...s,result:result(s)});}
...
test('server rejects altered difficulty, model, timings, tokens and inconsistent scores',async()=>{
  const f=await fixture();try{
    await assert.rejects(f.service.start({difficulty:'custom',rulesVersion:3,modelHash:'model-test'}));await assert.rejects(f.service.start({difficulty:'easy',rulesVersion:3,modelHash:'old-model'}));
    const s=await f.service.start({difficulty:'easy',rulesVersion:3,modelHash:'model-test'});const r=result(s);
    assert.equal(s.settings.durationMs,120000);
    await assert.rejects(f.service.start({difficulty:'easy',rulesVersion:2,modelHash:'model-test'}),/rules have changed/);
    await assert.rejects(f.service.finish({...s,result:r}),/duration/);f.add(120000);
...
test('ranking sorts by lines then pieces; aborted sessions are excluded',async()=>{
  const f=await fixture();try{await f.service.start({difficulty:'medium',rulesVersion:3,modelHash:'model-test'});
...
test('local scores persist after closing and reopening the database',async()=>{
  ...
  try{db=localDatabase(file);await migrate(db);let t=1000000;const service=makeService(db,'model-test',()=>t),s=await service.start({difficulty:'hard',rulesVersion:3,modelHash:'model-test'});t+=120000;await service.finish({...s,result:result(s)});db.close();db=localDatabase(file);
...
test('falling matches accept legal rapid controls and isolate legacy leaderboard rows',async()=>{
  const f=await fixture();try{
    const s=await f.service.start({difficulty:'medium',rulesVersion:3,modelHash:'model-test'});f.add(1000);
    ...
    assert.equal((await f.db.query('SELECT rules_version FROM flytris_matches WHERE id=$1',[s.id]))[0].rules_version,3);
    assert.equal((await f.service.leaderboard('all')).lists.medium[0].fly_pieces,10);
    await f.db.query('UPDATE flytris_matches SET rules_version=2 WHERE id=$1',[s.id]);
    ...
    const fresh=await f.service.start({difficulty:'medium',rulesVersion:3,modelHash:'model-test'});f.add(120000);
